...
| 3e | **64** | ✅ done — stage-3e wave: 22 branch-focused tests on `webhook-processor.ts` (485 lines, previously untested). Security-critical: replay-defense + cross-tenant resolution (tenant comes from `IntegrationConnection`, never from caller) + signature verification (github/gitlab/generic-hmac/no-secret-allow branches) + provider-impl dispatch fan-out (executions + evidence per triggeredKey × matching control) + best-effort orchestrator dispatch + sanitizeHeaders PII redaction. File-level **0/0/0/0 → 98/86/86/99**. Floor bumped +2 (target was ≈65; cautious after stage-3d's overshoot — back off to +1 if CI rejects). | |
