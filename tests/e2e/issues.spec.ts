...
test.describe('Issue Management', () => {
    ...
    test('issues list page loads with filters and CTA', async ({ page }) => {
        ...
        await expect(page.locator('#new-task-btn')).toBeVisible({ timeout: 5000 });
    ...
    test('create a new issue and see detail', async ({ page }) => {
        tenantSlug = await loginAndGetTenant(page);
        await page.goto(`/t/${tenantSlug}/tasks/new`);
        await page.waitForSelector('#task-title-input', { timeout: 10000 });
    ...
    test('dashboard page renders metrics', async ({ page }) => {
        tenantSlug = await loginAndGetTenant(page);
        await page.goto(`/t/${tenantSlug}/tasks/dashboard`);
